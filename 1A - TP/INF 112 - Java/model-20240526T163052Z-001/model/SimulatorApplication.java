...
public class SimulatorApplication {
    public static void main(String[] args) {
		...
		*/
        ...
        Room packagingRoom = new Room("Packaging Room", 5, 15, 30.0, 30.0);
        Room stockRoom = new Room("Stock Room", 40, 15, 30.0, 30.0);
        Room sortingRoom = new Room("Sorting Room", 75, 15, 20.0, 30.0);

        usine.addRoom(packagingRoom.getName(), packagingRoom.getxCoordinate(), packagingRoom.getyCoordinate(), packagingRoom.getHeight(), packagingRoom.getWidth());
        usine.addRoom(stockRoom.getName(), stockRoom.getxCoordinate(), stockRoom.getyCoordinate(), stockRoom.getHeight(), stockRoom.getWidth());
        usine.addRoom(sortingRoom.getName(), sortingRoom.getxCoordinate(), sortingRoom.getyCoordinate(), sortingRoom.getHeight(), sortingRoom.getWidth());
        ...
        roboCop.addComponentsToVisit(packagingRoom);
        roboCop.addComponentsToVisit(stockRoom);
        roboCop.addComponentsToVisit(sortingRoom);
            		
        
        terminator.addComponentsToVisit(stockRoom);
        terminator.addComponentsToVisit(sortingRoom);
        terminator.addComponentsToVisit(packagingRoom);
        
        wallE.addComponentsToVisit(sortingRoom);
        wallE.addComponentsToVisit(packagingRoom);
        wallE.addComponentsToVisit(stockRoom);


        SimulateurController controller = new SimulateurController(usine);
